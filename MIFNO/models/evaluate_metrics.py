...
from utils_models import get_device
...
parser.add_argument('@dir_logs', type=str, default='./MIFNO/logs/', help="Path to folder to store loss and models")
parser.add_argument('@name_config', type=str, default="MIFNO3D-moment-dv16-16layers-S32-T320-learningrate0p0004-Ntrain27000-batchsize8-normedsource-normedtraces", help="Name of model to load")
parser.add_argument('@epochs', type=int, default=600, help="Number of epochs used to train the model")
parser.add_argument('@dir_data_test', type=str, nargs='+', default=['./shared/hemews3d/formatted/HEMEWS3D_S32_Z32_T320_fmax5_rot0_val'], help="Name of folders with test data")
...
parser.add_argument('@batch_size', type=int, default=8, help="Number of samples used in the predictions")
parser.add_argument('@store_source', action='store_true', help="Store source params in metrics CSV")
...
if __name__ == '__main__':
    ...
    store_source = options.get('store_source', False)
    if store_source:
        all_s = torch.zeros((Ntest, 3), dtype=torch.float32, device=device)  # raw or normalized: choose
        all_m = torch.zeros((Ntest, 6), dtype=torch.float32, device=device)  # moment if present
    ...
    with torch.no_grad():        
        for _ in test_loader:
            ...
            if upsample:
                outE = torch.from_numpy(resample(outE.detach().cpu().numpy(), num=T_out, axis=-2)).to(device)
                outN = torch.from_numpy(resample(outN.detach().cpu().numpy(), num=T_out, axis=-2)).to(device)
                outZ = torch.from_numpy(resample(outZ.detach().cpu().numpy(), num=T_out, axis=-2)).to(device)
    ...
    df = pd.DataFrame(index=np.arange(Ntest), columns=[
        'rMAE','rRMSE','rFFTlow','rFFTmid','rFFThigh','EG','PG',
    ], dtype=float)
