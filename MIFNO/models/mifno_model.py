...
from MIFNO.models.utils_ffno import WNLinear, FactorizedSpectralConv3d, ModifyDimensions3d
...
class MIFNO_3D(nn.Module):
    def __init__(self, list_D1, list_D2, list_D3, list_M1, list_M2, list_M3, 
                 ...
                 branching_index=4, n_layers=16, padding=0, original=False):
        ...
        ### FINAL PROJECTIONS
        self.QE = nn.Sequential(WNLinear(3*self.width, 128, wnorm=True),
                                WNLinear(128, output_dim, wnorm=True))
        self.QN = nn.Sequential(WNLinear(3*self.width, 128, wnorm=True),
                                WNLinear(128, output_dim, wnorm=True))
        self.QZ = nn.Sequential(WNLinear(3*self.width, 128, wnorm=True),
                                WNLinear(128, output_dim, wnorm=True))
