...
from utils_models import get_batch_size, loss_criterion, RunningAverage, EarlyStopper
...
from utils_metrics import lowpass_torch_butter
...
parser.add_argument('@downsample_k', type=int, default=1, help="Downsample factor along time axis (1 = no downsample)")
...
if __name__ == '__main__':
    ...
    name_config = f"{model_type}3D-{source_orientation}-dv{dv}-{nlayers}layers-S{S_in}-T320-"\
    ...
    if restart_model != "":  # initialize weights with a trained model
        ckpt = torch.load(restart_model, map_location=device)
 
        # New format: dict with 'model', 'optimizer', etc.
        if isinstance(ckpt, dict) and 'model' in ckpt:
            ...
            if verbose:
                print(f"Resuming from epoch {start_epoch} (best_loss={best_loss:.5f})")
        else:
            model.load_state_dict(ckpt)
            if verbose:
                print("model loaded!")

    # ---- Train all params (you requested unfreeze everything) ----
    ...
    if verbose:
        print(f"Num trainable params: {sum(p.numel() for p in trainable_params):.2e}")
    ...
    reference_bs = 128
    optimizer = optim.Adam(trainable_params, lr=learning_rate * (global_bs / reference_bs), betas=(0.9, 0.999))

    lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, 'min', factor=0.5, patience=15, verbose=True
    )
 
    early_stopper = EarlyStopper(patience=patience, min_delta=0.0001)
 
    if restore_optim is not None:
        optimizer.load_state_dict(restore_optim['optimizer'])
        lr_scheduler.load_state_dict(restore_optim['scheduler'])
        if verbose:
            print(f"Optimizer and scheduler restored from checkpoint")
    ...
    for ep in range(start_epoch, epochs):
        ...
        if rank == 0:
            ...
            print(f"[epoch {ep+1}] lr = {lr:.3e}")
        train_losses_relative = RunningAverage()
        train_losses_absolute = RunningAverage()
        if rank == 0:
            pbar = tqdm(train_loader, desc=f"Epoch {ep+1}/{epochs}", ncols=120)
        else:
            pbar = train_loader
        ...
        for _ in pbar:
            ...
            k = options['downsample_k']
            uE_ds = uE[:, :, :, ::k, :]
            uN_ds = uN[:, :, :, ::k, :]
            uZ_ds = uZ[:, :, :, ::k, :]

            loss_rel = loss_criterion((outE, outN, outZ),
                                    (uE_ds, uN_ds, uZ_ds),
                                    loss_weights, relative=True)
            loss_abs = loss_criterion((outE, outN, outZ),
                                    (uE_ds, uN_ds, uZ_ds),
                                    loss_weights, relative=False)
        ...
        if rank == 0:
            ...
            with torch.no_grad():
                ...
                for _ in val_loader:
                    ...
                    # existing
                    loss_rel_val = loss_criterion((outE, outN, outZ),
                                            (uE_ds, uN_ds, uZ_ds),
                                            loss_weights, relative=True)
                    loss_abs_val = loss_criterion((outE, outN, outZ),
                                            (uE_ds, uN_ds, uZ_ds),
                                            loss_weights, relative=False)
            ...
            if val_losses_relative.avg < best_loss:
                ...
                torch.save(model.module.state_dict(), f'{dir_logs}models/bestmodel-{name_config}-epochs{epochs}.pt')
            ...
            }, f'{dir_logs}models/latest-{name_config}.pt')
    ...
    if rank == 0:
        ...
        # Remove latest checkpoint so auto-resubmit knows training is done
        latest_ckpt = f'{dir_logs}models/latest-{name_config}.pt'
        if os.path.exists(latest_ckpt):
            os.remove(latest_ckpt)
            print("Training complete — removed latest checkpoint")
