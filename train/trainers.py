...
class BaseTrainer(Generic[M, S], metaclass=abc.ABCMeta):
    ...
    def train(self, batch_iter: Iterator[BatchType], num_steps: int) -> Metrics:
        ...
        for step in range(num_steps):
            ...
            batch = next(batch_iter)                                  # dataloader
            new_batch = {}
            # Robust to dict
            for k, v in batch.items():
                if torch.is_tensor(v):
                    new_batch[k] = v.to(self.device, non_blocking=True)
                elif k == "cond" and isinstance(v, dict):
                    # move inner tensors of cond ONLY
                    inner = {}
                    for ck, cv in v.items():
                        if torch.is_tensor(cv):
                            inner[ck] = cv.to(self.device, non_blocking=True)
                        else:
                            inner[ck] = cv
                    new_batch[k] = inner
                else:
...
class DenoisingTrainer(BasicTrainer[M, SD]):
    ...
    @compute_memory
    def train_step(self, batch: BatchType) -> Metrics:
            p = batch["initial_cond"]        # prior, shape (B, C, 32, 32, 320)
            t = batch["target_cond"]         # target, same shape
            n = batch["norm_traces"]         # (B, 1, 1, 1, 1)
            cond_dict = batch.get("cond", None)

            scale = 1.0
            target_residual = t
            target_scaled = target_residual * scale
            prior_scaled = None if p is None else p * scale

            # Build a shallow copy of the batch that the model understands
            batch_scaled = dict(batch)
            
            batch_scaled["initial_cond"] = prior_scaled
            batch_scaled["target_cond"]  = target_scaled
            batch_scaled["cond"] = cond_dict if cond_dict is not None else None

            batch_scaled["scaled_space"] = True

            with torch.amp.autocast(device_type=self.device.type, dtype=self.compute_dtype):
                loss, metrics = self.model.loss_fn(batch_scaled)
            ...
            # -------- EMA update (cheap, happens once per step) -----------
            if self.store_ema:
