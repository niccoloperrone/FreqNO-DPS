...
"""Model argument assembly for the unconditional GenCFD denoiser"""
...
def reshape_jax_torch(tensor: Tensor, kernel_dim: int = None) -> Tensor:
  ...
  It transforms a tensor as follows:
  - 2D: (bs, c, height, width) <-> (bs, height, width, c)
  - 3D: (bs, c, depth, height, width) <-> (bs, depth, height, width, c)

  Code can be used either dynamics or static.
  - dynamic: if kernel_dim is None
  - static: if kernel_dim is specified
  """
...
def get_model_args(
    ...
    """Argument dictionary for the unconditional 3D U-Net denoiser"""
...
def get_denoiser_args(
    ...
    """Argument dictionary for DenoisingModel"""
    return {
        "spatial_resolution":     spatial_resolution,
        "time_cond":              time_cond,
        "denoiser":               denoiser,
